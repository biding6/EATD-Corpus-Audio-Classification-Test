...
# (collate_fn, train_one_epoch, evaluate 函数的核心逻辑保持不变)
...
def train_one_epoch(model, data_loader, loss_fn, optimizer, scheduler, device):
    ...
    for waveforms, sample_rate, labels in tqdm(data_loader, desc="Training Audio_Flamingo_2"):
...
if __name__ == "__main__":
    ...
    print(f"Total number of trainable parameters for Audio_Flamingo_2 (Head): {sum(p.numel() for p in trainable_params)}")
